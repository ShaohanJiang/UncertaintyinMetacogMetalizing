...
% shif_interval = [-bar_interval/2, 0, bar_interval/2];
shift_span=(1-bar_width)*(N_grps-1);
bar_shift=linspace(-shift_span/2,+shift_span/2,N_grps);
if mod(N_grps,2) == 0
    half_shift = (1:N_grps/2)-0.5;
    shif_interval = sort([half_shift*bar_interval, -1*half_shift*bar_interval]);
else
    half_shift = 1:floor(N_grps/2);
    shif_interval = sort([half_shift*bar_interval, 0, -1*half_shift*bar_interval]);
end
bar_shift = bar_shift - shif_interval;
